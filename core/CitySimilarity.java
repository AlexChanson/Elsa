...
public class CitySimilarity {
    ...
    public Double calculateDistance(Commune commune1, Commune commune2){
        ...
        double actifsDist = divisor == 0? 1 : Math.pow((commune1.getActifs_2010()-commune2.getActifs_2010())/
                divisor,2.0);

        divisor = Math.max(commune1.getNb_actifs_2015(), commune2.getNb_actifs_2015());
        double actifs2015Dist = divisor == 0? 1 : Math.pow((commune1.getNb_actifs_2015()-commune2.getNb_actifs_2015())/divisor,2);

        divisor = Math.max(commune1.getNb_etablissements(), commune2.getNb_etablissements());
        double publicEtDist = divisor == 0? 1 : Math.pow((commune1.getNb_etablissements()-commune2.getNb_etablissements())/
                divisor,2);

        divisor = Math.max(commune1.getNb_etudiants(), commune2.getNb_etudiants());
        double nbEtuDist = divisor == 0? 1 : Math.pow((commune1.getNb_etudiants()-commune2.getNb_etudiants())/divisor,2);
        ...
        double urbaniteDist = divisor == 0? 1 : Math.pow((commune1.getScore_urbanite()-commune2.getScore_urbanite())/
                divisor,2);
